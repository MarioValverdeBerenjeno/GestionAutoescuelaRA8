...
public class UsuarioService {
	...
	public void save(Connection conexion, Usuario user) throws SQLException {
		...
		try {
			...
			if (user.getId() == 0) {
				consulta = conexion
						.prepareStatement("INSERT INTO " + this.tabla + "(nombre, contrasenya, rol) VALUES(?, ?, ?)");
				...
				consulta = conexion.prepareStatement(
						"UPDATE " + this.tabla + " SET nombre = ?,contrasenya = ?, rol = ? WHERE idUsuario = ?");
				consulta.setString(1, user.getNombre());
				consulta.setString(2, user.getPassword());
				consulta.setString(3, user.getRol());
	...
	public Usuario getUsuario(Connection conexion, int idUsuario) throws SQLException {
		...
		try {
			PreparedStatement consulta = conexion.prepareStatement(
					"SELECT nombre,contrasenya,rol " + " FROM " + this.tabla + " WHERE idUsuario = ?");
	...
	public Usuario getUsuarioNombre(Connection conexion, String nombre) throws SQLException {
		...
		try {
			PreparedStatement consulta = conexion.prepareStatement(
					"SELECT idUsuario,nombre,contrasenya,rol " + " FROM " + this.tabla + " WHERE nombre = ?");
			consulta.setString(1, nombre);
			ResultSet resultado = consulta.executeQuery();
			while (resultado.next()) {
				user = new Usuario(resultado.getInt("idUsuario"), resultado.getString("nombre"),
	...
	public void remove(Connection conexion, Usuario user) throws SQLException {
		try {
			PreparedStatement consulta = conexion
					.prepareStatement("DELETE FROM " + this.tabla + " WHERE idUsuario = ?");
